...
ImagePixels ImageUtils::LoadDxPBgra32Image(const Microsoft::WRL::ComPtr<IStream> &stream, uint32_t maxSize) {
    ...
    return ImageUtils::LoadDxPBgra32Image(dec, maxSize);
}

ImagePixels ImageUtils::LoadDxPBgra32Image(const Microsoft::WRL::ComPtr<IWICBitmapDecoder> &dec, uint32_t maxSize) {
    HRESULT hr = S_OK;
    ImagePixels pixels;
    ImageUtils imgUtils;
